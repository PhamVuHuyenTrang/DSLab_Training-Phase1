...
class Kmeans:
    ...
    def random_init(self, seed_value):
        ...
        for i in range(self._num_clusters):
            ...
            cluster_i.set_centroid(centroid[i])
    def compute_similarity(self, member, centroid):
        return math.dist([member._r_d], [centroid])
...
def compute_accurracy(predicted_y, expected_y):
    matches = np.equal(predicted_y, expected_y)
    accuracy = np.sum(matches.astype(float)) / expected_y.size
    return accuracy
 
K = Kmeans(num_clusters = 20)
K.load_data('D:/DSLab/DSLab_Training_Phase1/Dataset/20news-bydate/data_tf_idf.txt')
