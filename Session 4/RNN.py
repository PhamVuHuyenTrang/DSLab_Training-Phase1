...
class DataReader:
    def __init__(self, data_path, batch_size, vocab_size):
        ...
        self._final_tokens = np.array(self._final_tokens, dtype = object)
...
def train_and_evaluate_RNN():
    with open("D:/DSLab/DSLab_Training_Phase1/Dataset/20news-bydate/vocab-raw.txt") as f:
    ...
    with tf.Session() as sess:
        train_data_reader = DataReader(data_path = "D:/DSLab/DSLab_Training_Phase1/Dataset/20news-bydate/20news-train-encoded.txt", batch_size = 50, vocab_size = vocab_size)
        test_data_reader = DataReader(data_path = "D:/DSLab/DSLab_Training_Phase1/Dataset/20news-bydate/20news-test-encoded.txt", batch_size = 50, vocab_size = vocab_size)
        step = 0
        MAX_STEP = 1000 ** 2
        ...
        while step < MAX_STEP:
            ...
            if train_data_reader._batch_id == 0:
                num_true_preds = 0
                while True:
                    next_train_batch = test_data_reader.next_batch()
                    test_data, test_labels, test_sentence_lengths, test_final_tokens = next_train_batch
                    test_plabels_eval = sess.run(predicted_labels,
                                                  feed_dict = {
                                                      rnn._data: test_data,
                                                      rnn._label: test_labels,
